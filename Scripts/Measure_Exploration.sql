...
#for creating a proper table of KPIS:
-- Generate a Report that shows all key metrics of the business

SELECT 'Total Sales' AS measure_name, 
       SUM(quantity * price) AS measure_value 
FROM fact_sales
...
SELECT 'Total Orders (All Rows)' AS measure_name, 
       COUNT(order_number) AS measure_value 
FROM fact_sales

UNION ALL

SELECT 'Total Distinct Orders' AS measure_name, 
...
SELECT 'Customers Who Placed Orders' AS measure_name, 
       COUNT(DISTINCT customer_key) AS measure_value 
FROM fact_sales;
